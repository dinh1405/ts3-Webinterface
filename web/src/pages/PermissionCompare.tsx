...
import { Badge, Button, Card, ConfirmDialog, EmptyState, ErrorBox, Field, FullPageSpinner, PageHeader, Toggle, Alert } from '../components/ui';
...
const STATUS_TONE: Record<DiffStatus, 'success' | 'danger' | 'warning' | 'neutral'> = { added: 'success', removed: 'danger', changed: 'warning', same: 'neutral' };
...
export default function PermissionComparePage() {
  ...
  return (
    <div>
      ...
      {diff && (
        <Card noPadding
          title={<span className="flex flex-wrap items-center gap-2">{t('perms.compare.result')}<Badge tone="success">{t('perms.compare.added', { count: diff.added })}</Badge><Badge tone="danger">{t('perms.compare.removed', { count: diff.removed })}</Badge><Badge tone="warning">{t('perms.compare.changed', { count: diff.changed })}</Badge><Badge tone="neutral">{t('perms.compare.same', { count: diff.same })}</Badge></span>}
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.name} className={clsx(r.status === 'added' && 'bg-emerald-500/5', r.status === 'removed' && 'bg-rose-500/5', r.status === 'changed' && 'bg-amber-500/5')}>
                      ...
                      <td className="font-mono text-xs">{r.a ? fmt(r.a) : <span className="text-slate-500">–</span>}</td>
                      <td className="font-mono text-xs">{r.b ? fmt(r.b) : <span className="text-slate-500">–</span>}</td>
      ...
      <ConfirmDialog open={confirm !== null} onClose={() => setConfirm(null)} onConfirm={() => confirm && copy.mutate(confirm)} loading={copy.isPending} tone={confirm === 'replace' ? 'warning' : 'primary'}
        ...
        message={<div className="space-y-2">
          ...
          {confirm === 'replace' && sensitive && <Alert tone="warning" compact>{t('perms.tools.sensitiveWarning', { name: nameB })}</Alert>}
