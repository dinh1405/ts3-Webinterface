...
import { Badge, Button, Card, ConfirmDialog, ErrorBox, Field, FullPageSpinner, Toggle, Alert, StatusText } from './ui';
...
export function AutoUpdateTab() {
  ...
  const describe = (c: AutoUpdateComponent | null | undefined, busy: boolean): { text: string; tone: 'success' | 'danger' | 'warning' | 'neutral' } => {
    if (busy) return { text: t('system.auto.sum.busy'), tone: 'warning' };
    if (!c) return { text: t('system.auto.sum.disabled'), tone: 'neutral' };
    if (c.skipped) {
      ...
      return { text: td(`system.auto.sum.${key}`, params), tone: key === 'upToDate' || key === 'disabled' || key === 'noDir' ? 'neutral' : 'warning' };
    }
    if (c.ok) return { text: t(c.restart ? 'system.auto.sum.updatedRestart' : 'system.auto.sum.updated', { from: c.from ?? '?', to: c.to ?? '?' }), tone: 'success' };
    return { text: t('system.auto.sum.failed', { from: c.from ?? '?', to: c.to ?? '?', error: c.error ?? '?' }), tone: 'danger' };
  };
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <Card title={<span className="flex items-center gap-2"><CalendarClock className="h-4 w-4 text-indigo-400" /> {t('system.auto.title')}</span>} subtitle={t('system.auto.subtitle')}>
        <div className="space-y-4">
          ...
          {s.ts3 && !info.ts3Configured && <Alert tone="warning" compact>{t('system.auto.ts3NotConfigured')}</Alert>}
          {s.webinterface && !info.selfUpdate.canUpdate && (
            <Alert tone="warning" compact title={t('system.auto.selfNotPossible')}>
              <ul className="list-disc pl-5">{info.selfUpdate.reasons.map((r) => <li key={r}>{td(`system.self.reason.${r}`, undefined, r)}</li>)}</ul>
            </Alert>
          )}
          {s.webinterface && info.selfUpdate.canUpdate && info.selfUpdate.restartMode !== 'systemd' && <StatusText tone="warning" className="text-xs">{t('system.auto.restartManual')}</StatusText>}
      ...
      <Card title={t('system.auto.lastRun')} subtitle={info.nextRun ? `${t('backups.nextRun')}: ${formatDate(info.nextRun)} (${formatRelative(info.nextRun)})` : t('backups.notScheduled')} actions={info.running && <RefreshCw className="h-4 w-4 animate-spin text-indigo-400" />}>
        ...
        {info.running && (
          <Alert tone="accent" compact spinning icon={RefreshCw} className="mb-3">
        ...
        {!last ? <p className="text-sm text-slate-500">{t('system.auto.noRun')}</p> : (
          <div className="space-y-3 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              {(() => { const acted = [last.ts3, last.webinterface].some((c) => c && c.ok !== undefined); return <Badge tone={!last.ok ? 'danger' : acted ? 'success' : 'neutral'}>{!last.ok ? t('system.upd.failed') : acted ? t('system.upd.success') : t('system.auto.nothingToDo')}</Badge>; })()}
            ...
            {last.skipped === 'busy' && <StatusText tone="warning">{t('system.auto.busy')}</StatusText>}
              ...
              {(['ts3', 'webinterface'] as const).map((k) => {
                ...
                return (
                  <li key={k} className="flex items-start gap-3 rounded-lg bg-slate-950/60 px-3 py-2">
                    ...
                    <StatusText tone={d.tone} className={d.tone === 'neutral' ? 'text-slate-300' : undefined}>{d.text}</StatusText>
                  </li>
                );
              })}
            </ul>
